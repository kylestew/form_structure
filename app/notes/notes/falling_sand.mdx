...
<Canvas id="example_weighted_rules" width={200} height={300} />

_click to restart_

Give sand particles a random weight on generation. When it collides with a particle of less weight, it will attempt to push that particle asside. Shade the heavier particles to be darker.

```javascript
// @meta store:weightedRules
function weightedRules(idx, belowIdx, leftIdx, rightIdx, grid) {
    // // attempt to push particles to the left or right
    // if (!isEmptyFn(belowIdx)) {
    //     // get the weight of the particle below
    //     // TODO: this isn't working
    //     const weight = getWeightFn(idx)
    //     const belowWeight = getWeightFn(belowIdx)
    //     if (belowWeight < weight) {
    //         // Try to push to left or right randomly
    //         if (Math.random() < 0.5 && leftIdx > -1 && isEmptyFn(leftIdx)) {
    //             swapFn(belowIdx, leftIdx)
    //             swapFn(idx, belowIdx)
    //         } else if (rightIdx > -1 && isEmptyFn(rightIdx)) {
    //             swapFn(belowIdx, rightIdx)
    //             swapFn(idx, belowIdx)
    //         } else {
    //             swapFn(idx, belowIdx)
    //         }
...
function animate() {
    ...
    // randomly add new sand 33% of the time
    if (Math.random() < 0.33) {
        const weight = Math.floor(Math.random() * 100) + 50 // between 50-150
        // Map weight (50-150) to color brightness (0.5-1.0)
        const brightness = 0.5 + ((weight - 50) / 100) * 0.5
        const color = palette[3].map((c) => c * brightness)
        grid.set(grid.width / 2, 12, weight, color)
    }
